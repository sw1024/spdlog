// Copyright(c) 2015-present, SunWu
...
{
public:
    // create hour file sink which rotates on given time
    hour_file_sink(filename_t base_filename, bool truncate = false, uint16_t max_files = 0)
    ...
    void delete_old_()
    {
        ...
        if (filenames_q_.full())
        {
            ...
            if (!ok)
            {
                ...
                throw_spdlog_ex("Failed removing hour file " + filename_to_str(old_filename), errno);
            }
        }
        filenames_q_.push_back(std::move(current_file));
    }
...
{
    return Factory::template create<sinks::hour_file_sink_mt>(logger_name, filename, truncate, max_files);
}
...
inline std::shared_ptr<logger> hour_logger_st(
    const std::string &logger_name, const filename_t &filename, bool truncate = false, uint16_t max_files = 0)
{
    return Factory::template create<sinks::hour_file_sink_st>(logger_name, filename, truncate, max_files);
}
} // namespace spdlog
